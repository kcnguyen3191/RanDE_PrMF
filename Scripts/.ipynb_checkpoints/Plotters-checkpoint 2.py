...
#     res = ax.imshow(weights_mesh.T, aspect='auto',extent=[param1_min, param1_max, param2_max, param2_min], alpha=1)
...
#         plt.plot(x, U_data[:,t_idx], marker=markers[t_idx//5], c=colors[t_idx//5], linestyle='--')
    ...
    plt.ylabel('Cel density', fontsize=14)
...
def Plot_WaveSpeedComparison(densities,
    ...
    if ylim is not None:
        ax.set_ylim(ylim)
    ax.legend(['True','RDE','2-PDE', '4-PDE', '6-PDE'],loc='upper right')
    # set y-axis label
